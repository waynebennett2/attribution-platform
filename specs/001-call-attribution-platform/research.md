...
- **Decision**: Standard OpenID Connect authorization-code flow against the customer's identity provider; on successful federation the platform issues its own JWT carrying the mapped role (FR-046), expiring in 5 minutes, with the client silently refreshing it against the still-active IdP browser session before each expiry — rather than passing the IdP's token through to downstream calls, and rather than a per-request revocation check against the IdP. Break-glass accounts (2 by default, FR-046) are local username/password + TOTP MFA records, checked only when OIDC discovery/token endpoints are unreachable.
- **Rationale (token lifetime)**: SC-016 requires a revoked user to lose access without waiting for a longer-lived session to expire. A 5-minute JWT bounds that staleness window tightly while keeping every API request's auth check a cheap, stateless signature verification — no per-request call out to the IdP, consistent with the stateless, horizontally-scalable API design (FR-043).
