...
7. **Given** a signed-in user whose access the customer's identity provider then withdraws, **When** their current short-lived token's next refresh is attempted (within 5 minutes, FR-046), **Then** access is refused rather than waiting for a longer-lived session to expire; **and Given** the identity provider is unreachable, **When** a break-glass administrator account signs in with multi-factor authentication, **Then** access is granted and the sign-in is audited and surfaced as an exceptional event.
...
- **FR-002**: System MUST allow numbers to be imported into a pool in bulk from a CSV file, rejecting duplicates and malformed entries with a per-entry reason. A malformed entry is one whose number field is empty, contains characters other than digits and an optional leading `+`, or is not a syntactically valid E.164 number (country code plus subscriber number, 8–15 digits total) — any row failing this check is rejected with that specific reason rather than silently coerced or accepted.
...
- **SC-016**: Withdrawing a user at the customer's identity provider removes their platform access within 5 minutes (one token refresh interval, FR-046) with no separate action taken in the platform, and zero passwords exist for federated users — verified by disabling a signed-in test user centrally, confirming their access is refused once their current token expires, and inspecting the stored user records.
