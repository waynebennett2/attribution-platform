...
- **FR-012**: System MUST support a configurable session timeout and heartbeat interval per website, so that sessions expire predictably.
...
- **FR-040**: System MUST support a configurable data retention period per data category, and MUST purge data automatically once it expires, except where it is still referenced by an open manual review case.
...
- **SC-001**: During a parallel run alongside the existing Mediahawk system, at least 95% of eligible calls are attributed to the same session that Mediahawk attributes them to, where an eligible call is an inbound call to a tracking number that the platform records as allocated to a session at the call's start time.
...
- **SC-004**: A visitor's tracking number is determined within 300 milliseconds for at least 95% of page loads, sustained at a peak of 250 concurrent tracked sessions.
...
- The existing Mediahawk system remains live throughout the parallel run and its attribution output is available for comparison; without it SC-001 cannot be measured.
...
- With a 30-minute attribution window and a cooldown of equal length, a number is unavailable for roughly an hour after the session holding it ends. A pool must therefore contain at least as many active numbers as its peak concurrent sessions plus every session that ended in the preceding hour, plus headroom. Sizing each pool is an operational responsibility of the deploying customer; the platform reports exhaustion rather than preventing it.
- A call's connected duration as reported by the source is authoritative for qualification.

**Operations and expected scale**

- The platform is sized for a peak of approximately 250 concurrent tracked sessions across all websites. Combined with the 30-minute attribution window and equal cooldown, and assuming an average session of around five minutes, this implies a total tracking number estate of roughly 3,000 DIDs. That figure is a direct and linear consequence of the window length: shortening the window reduces it proportionally, and lengthening the window increases it proportionally. Monthly call volume and the number of websites and pools were not stated and should be confirmed during technical planning, as they affect ingestion sizing and report query cost but not pool sizing.
- Retention periods default to a duration that covers the longest external conversion window plus reporting needs, and are configurable per data category.
...
- Because attribution now begins at consent rather than at arrival, measured attributed call volume will be lower than Mediahawk's if Mediahawk tracks pre-consent. This affects volume comparisons during the parallel run but not SC-001, which measures agreement on eligible calls rather than total counts.
