...
- [ ] T162 *(redefined 2026-09-08, same day — replaces the original actor-identity-injection task, research.md §22; role list extended 2026-09-08 to four roles per attribution-ui)* Create the native MySQL roles in `db/access-roles/` — `role_system_administrator`, `role_admin_reports`, `role_marketing_administrator`, `role_analyst` (the fourth, `role_admin_reports`, was added directly against `attribution-ui` at the project owner's request: everything `role_system_administrator` has, plus reporting access, for an admin user who also needs report visibility — see `attribution-ui/server/README.md`'s `CREATE ROLE`/`GRANT` script, which is this task's actual current implementation and should be reconciled into this repo's own `db/access-roles/` files rather than left living only in the UI repo) — each granted `EXECUTE`/`SELECT` on exactly the procedures/views its Apiche-era Role column already specified (`apiche-config.md`'s existing per-endpoint assignments, reused unchanged as the permission mapping, now also noting `AdminReports` as a valid `sp_create_user`/`sp_override_user_role` role value), and confirm the account-provisioning flow: `sp_create_user` creates a native database account and grants it the matching role, `sp_deactivate_user` disables the account, `sp_override_user_role` re-grants it. Audit actor identity is resolved via `CURRENT_USER()` inside each procedure — no injection mechanism is needed, which is what makes this task safe to build directly rather than escalate first (contrast with T208, which stays a genuine open blocker for the 4 DNI endpoints).
...
## Attribution-UI Integration and Live RBAC Verification (2026-09-08, deferred)

**Purpose**: `attribution-ui` (a separate repo, `../attribution-ui` alongside this one) is the trusted internal client this phase's direct-database-access operations were designed for — its `server/` directory is a working PHP implementation of the Admin/Reporting access layer T161–T201 above describe, built and code-reviewed directly against `apiche-config.md` during this same session, including the fourth role (`AdminReports`/"Admin-Reports") added at the project owner's request. What remains is reconciling that implementation back into this repo's own tracked deliverables, and the live verification neither repo has been able to run yet, since the shared remote MySQL server (`83.147.29.30`, per `appsettings.Development.local.json`) refused this session's connection attempt on a host-allowlist basis (`Host '<reverse-dns-hostname>' is not allowed to connect`) — **the project owner explicitly deferred this until Apiche itself is configured**, rather than opening the server to a one-off residential IP now.

- [ ] T212 Reconcile `attribution-ui/server/README.md`'s `CREATE ROLE`/`GRANT` script — the actual, currently-working implementation of T162's four native roles — into this repo's own `db/access-roles/role_system_administrator.sql`, `role_admin_reports.sql`, `role_marketing_administrator.sql`, `role_analyst.sql` (plan.md's Target Project Structure already names these files; they don't exist yet). Also reconcile `attribution-ui/server/operations/admin.php` and `operations/reports.php` against `apiche-config.md` one more time — they were hand-transcribed and are believed current as of this session, but haven't been diffed against any later `apiche-config.md` edits.
- [ ] T213 Grant network access to whichever machine(s) will run this verification against the shared remote MySQL server (`83.147.29.30`) — a `CREATE USER '<user>'@'<host-or-ip>' ...` / `GRANT ... TO '<user>'@'<host-or-ip>'` on the existing `attr_user` account or a dedicated one, scoped to the `attribution` schema only, per this session's access-request note (blocked/deferred, see Purpose above)
- [ ] T214 Apply `db/schema/*.sql` (T159's translated migrations) and `db/procedures/*.sql` (`apiche-config.md` Appendix A, all 29 procedures) against the shared server, if not already current there (depends on T213)
- [ ] T215 Run the reconciled `CREATE ROLE`/`GRANT` script from T212 against the shared server (depends on T212, T214)
- [ ] T216 [P] Create one native MySQL account per role for live verification — System Administrator, Admin-Reports, Marketing Administrator, Analyst — via `sp_create_user` plus the matching native `CREATE USER`/`GRANT` (server/README.md's manual step, T163's still-open automation gap) (depends on T215)
- [ ] T217 Click through `attribution-ui` as each of the four T216 accounts and confirm: System Administrator sees Administration only (no Reporting nav, direct `/reports/*` URLs redirect away); Admin-Reports sees both; Marketing Administrator sees its existing subset plus Reporting; Analyst sees Reporting only (no Administration nav, direct `/admin/*` URLs redirect away) — the live counterpart to this session's `tsc -b`/`php -l`/code-level verification, which confirmed the logic but not actual behavior against a real database and a real browser (depends on T216)

**Checkpoint**: deferred by the project owner (2026-09-08) until Apiche is configured — resume at T213 once that's ready, or once another path to the shared MySQL server is available.
